...
run_arch_block(Z3Visitor *visitor, const IR::ConstructorCallExpression *cce,
...
MainResult create_state(Z3Visitor *visitor, const ParamInfo &param_info) {
    ...
    for (const auto &mapping : param_mapping) {
        ...
        if (const auto *cce = arg_expr->to<IR::ConstructorCallExpression>()) {
            ...
            visitor->visit(cst);
            if (const auto *z3_val = visitor->get_state()
                                         ->get_expr_result()
                                         ->to<ValContainer>()) {
                merged_vec.insert(
                    {param->name.name,
                     {{{param->name.name, *z3_val->get_val()}}, param_type}});
            } else {
