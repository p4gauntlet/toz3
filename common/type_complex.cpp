...
===============================================================================
***/
StructBase::StructBase(P4State* state, const IR::Type* type, cstring name, uint64_t /*member_id*/)
    : P4Z3Instance(type),
      state(state),
...
StructBase::StructBase(const StructBase& other) : P4Z3Instance(other), valid(other.valid) {
    width = other.width;
    state = other.state;
    ...
    for (auto& member_tuple : members) {
        ...
        if (const auto* sub_list = input_val->to<ListInstance>()) {
            if (auto* sub_target = target_val->to_mut<StructBase>()) {
                ...
                BUG("Unsupported set list class %s.", target_val->get_static_type());
            }
        } else {
...
z3::expr StructBase::operator==(const P4Z3Instance& other) const {
    ...
    P4C_UNIMPLEMENTED("Comparing a struct base to %s is not supported.", other.get_static_type());
}

z3::expr StructBase::operator!=(const P4Z3Instance& other) const { return !(*this == other); }
...
StructInstance::StructInstance(P4State* state, const IR::Type_StructLike* type, cstring name,
                               uint64_t member_id)
...
HeaderInstance::HeaderInstance(P4State* state, const IR::Type_Header* type, cstring name,
                               uint64_t member_id)
...
z3::expr HeaderInstance::operator==(const P4Z3Instance& other) const {
    ...
    P4C_UNIMPLEMENTED("Comparing a header to %s is not supported.", other.get_static_type());
}

z3::expr HeaderInstance::operator!=(const P4Z3Instance& other) const { return !(*this == other); }
...
StackInstance::StackInstance(P4State* state, const IR::Type_Stack* type, cstring name,
    ...
    : IndexableInstance(state, type, name, member_id),
      ...
      lastIndex(Z3Int(state, 0)),
      size(Z3Int(state, type->getSize())),
      int_size(type->getSize()),
...
HeaderUnionInstance::HeaderUnionInstance(P4State* state, const IR::Type_HeaderUnion* type,
    ...
    for (const auto* field : type->fields) {
        ...
        if (resolved_type->is<IR::Type_Header>()) {
            ...
            BUG_CHECK(si, "Unexpected generated instance %s", member_var->to_string());
            width += si->get_width();
            flat_id += si->get_width();
            insert_member(field->name.name, member_var);
            member_types.insert({field->name.name, resolved_type});
...
std::vector<std::pair<cstring, z3::expr>> HeaderUnionInstance::get_z3_vars(
    cstring prefix, const z3::expr* valid_expr) const {
    // TODO: Fix this.
...
void HeaderUnionInstance::isValid(Visitor* /*visitor*/, const IR::Vector<IR::Argument>* /*args*/) {
...
void HeaderUnionInstance::update_validity(const HeaderInstance* /*child*/,
    ...
    for (auto& member : members) {
        ...
        // TODO: Test this...
        hi->valid = z3::ite(valid_val, state->get_z3_ctx()->bool_val(false), *old_valid);
    }
}
...
z3::expr EnumBase::operator==(const P4Z3Instance& other) const {
    ...
    P4C_UNIMPLEMENTED("Comparing a enum base to %s is not supported.", other.get_static_type());
}

z3::expr EnumBase::operator!=(const P4Z3Instance& other) const { return !(*this == other); }
...
EnumBase::EnumBase(const EnumBase& other)
    : StructBase(other), ValContainer(other.val), member_type(other.member_type) {}
...
EnumInstance::EnumInstance(P4State* p4_state, const IR::Type_Enum* type, cstring name,
                           uint64_t member_id)
...
ErrorInstance::ErrorInstance(P4State* p4_state, const IR::Type_Error* type, cstring name,
                             uint64_t member_id)
    : EnumBase(p4_state, type, name, member_id) {
    // FIXME: Enums should not be a struct base, actually
    width = INT_WIDTH;
...
SerEnumInstance::SerEnumInstance(P4State* p4_state,
...
P4Z3Instance* SerEnumInstance::operator&(const P4Z3Instance& other) const {
    ...
    const auto* tb = p4_type->checkedTo<IR::Type_SerEnum>()->type->checkedTo<IR::Type_Bits>();
...
void unroll_list(const StructBase* input_struct, std::vector<P4Z3Instance*>* target_list) {
...
z3::expr ListInstance::operator==(const P4Z3Instance& other) const {
    ...
    P4C_UNIMPLEMENTED("Comparing a ListInstance to %s is not supported.", other.get_static_type());
}

z3::expr ListInstance::operator!=(const P4Z3Instance& other) const { return !(*this == other); }
...
TupleInstance::TupleInstance(P4State* state, const IR::Type_Tuple* type, cstring name,
                             uint64_t member_id)
    : IndexableInstance(state, type, name, member_id) {
    size_t idx = 0;
...
P4Z3Instance* TupleInstance::get_member(const z3::expr& index) const {
    ...
    P4C_UNIMPLEMENTED("Runtime indices not supported for %s", get_static_type());
}
