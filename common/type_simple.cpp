...
z3::expr pure_bv_cast(const z3::expr &expr, const z3::sort &dest_type) {
    ...
    if (expr.is_bv()) {
        ...
        BUG("Casting %s to a bit vector is not supported.", expr.to_string().c_str());
    }
...
===============================================================================
***/
Z3Bitvector::Z3Bitvector(const P4State *state, const IR::Type *p4_type, const z3::expr &val,
                         bool is_signed)
...
P4Z3Instance *Z3Bitvector::cast_allocate(const IR::Type *dest_type) const {
    ...
    P4C_UNIMPLEMENTED("cast to %s not implemented for %s.", dest_type->node_type_name(),
                      get_static_type());
}
...
P4Z3Instance *Z3Int::operatorAddSat(const P4Z3Instance &other) const {
    if (const auto *other_val = other.to<Z3Bitvector>()) {
        auto cast_val = pure_bv_cast(val, other_val->get_val()->get_sort());
        auto no_overflow = z3::bvadd_no_overflow(cast_val, *other_val->get_val(), false);
...
z3::expr Z3Int::operator!=(const P4Z3Instance &other) const { return !(*this == other); }
...
P4Z3Instance *Z3Int::cast_allocate(const IR::Type *dest_type) const {
    ...
    P4C_UNIMPLEMENTED("cast not implemented for %s to type %s.", get_static_type(),
