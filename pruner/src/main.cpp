...
const IR::P4Program *prune(const IR::P4Program *program, const PrunerConfig &pruner_conf,
                           uint64_t prog_size) {
    program = prune_statements(program, pruner_conf, prog_size);
    program = prune_expressions(program, pruner_conf);
    program = prune_bool_expressions(program, pruner_conf);
    program = apply_compiler_passes(program, pruner_conf);
    return program;
}

void process_seed(const PrunerOptions &options) {
    uint64_t seed = 0;
    if (options.seed.has_value()) {
        ...
        try {
            ...
            P4::error("invalid seed %s", options.seed.value());
            exit(EXIT_FAILURE);
        }
    } else {
...
PrunerConfig get_config_from_json(const std::filesystem::path &json_path,
                                  const std::filesystem::path &working_dir,
                                  const std::filesystem::path &input_file,
                                  const PrunerOptions &options) {
    if (!std::filesystem::exists(json_path)) {
        P4::error("Config file %s does not exist! Exiting.", json_path);
        exit(EXIT_FAILURE);
    }
...
PrunerConfig get_conf_from_script(const PrunerOptions &options) {
    INFO("Grabbing config by using the validation script.");
    if (!std::filesystem::exists(options.validation_bin.value())) {
        P4::error("Path to validator binary %s invalid! Exiting.", options.validation_bin.value());
        exit(EXIT_FAILURE);
    }
    if (!std::filesystem::exists(options.compiler_bin.value())) {
        P4::error("Path to compiler binary %s invalid! Exiting.", options.compiler_bin.value());
        exit(EXIT_FAILURE);
    }
    ...
    if (ret == 0) {
        P4::warning("There was no error. Pruning will yield bogus results.");
    }
...
PrunerConfig get_conf_from_compiler(const PrunerOptions &options) {
    ...
    auto exit_info = get_exit_info(options.file.c_str(), pruner_conf);
...
int main(int argc, char *const argv[]) {
    P4::AutoCompileContext autoP4toZ3Context(new P4::ToZ3::Pruner::P4PrunerContext);
    auto &options = P4::ToZ3::Pruner::P4PrunerContext::get().options();
    ...
    P4::ToZ3::Pruner::ErrorType error_type;

    if (!options.bug_type.has_value() && !options.config_file.has_value()) {
        P4::error("Please provide a valid bug type or a config file");
        options.usage();
        return EXIT_FAILURE;
    }
    P4::ToZ3::Pruner::PrunerConfig pruner_conf;
    if (options.config_file.has_value()) {
        ...
        if (options.bug_type.value() == "CRASH") {
            ...
            error_type = P4::ToZ3::Pruner::ErrorType::CrashBug;
        } else if (options.bug_type.value() == "VALIDATION") {
            error_type = P4::ToZ3::Pruner::ErrorType::SemanticBug;
        } else {
            P4::error(
        ...
        if (error_type == P4::ToZ3::Pruner::ErrorType::SemanticBug) {
        ...
        } else if (error_type == P4::ToZ3::Pruner::ErrorType::CrashBug) {
            if (!options.compiler_bin.has_value()) {
                P4::error("Need to provide a compiler binary to prune a crash bug");
                options.usage();
                return EXIT_FAILURE;
            }
    ...
    P4::ToZ3::Pruner::ExitInfo exit_info;
    ...
    pruner_conf.err_type = classify_bug(exit_info);
    ...
    try {
        ...
        if (program != nullptr && P4::errorCount() == 0) {
            ...
            double prog_size = P4::ToZ3::Pruner::count_statements(original);
            INFO("Size of the program :" << prog_size << " statements");

            program = prune(program, pruner_conf, prog_size);
            if (options.print_pruned) {
                P4::ToZ3::Pruner::print_p4_program(program);
            }
            // sometimes we do not want to emit the final file
            if (!options.dry_run) {
                P4::ToZ3::Pruner::emit_p4_program(program, pruner_conf.out_file_name);
            }
            INFO("Total reduction percentage = " << P4::ToZ3::Pruner::measure_pct(original, program)
                                                 << " %");
        }
        INFO("Done.");
    ...
    return static_cast<int>(P4::errorCount() > 0);
}
