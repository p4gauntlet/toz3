...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                ValueDeclaration("undef", None, z3_type=z3.BitVecSort(8)),
                MethodCallStmt(MethodCallExpr(P4Member("reset_0", "apply"), [], P4Member(P4Member("hdr", "eth_hdr"), "src_addr"), )),
                IfStatement(P4eq("undef", 2), BlockStatement([
                    AssignmentStatement(P4Member(P4Member("hdr", "eth_hdr"), "src_addr"), 1),]
                ), P4Noop()),]
            ),
            local_decls=[
