...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr("do_action_ingress_0", [], )),]
            ),
            local_decls=[
P4Declaration("do_action_0", P4Action("do_action", params=[
                    P4Parameter("inout", "val_0", z3.BitVecSort(8), None),],                 body=BlockStatement([
                    AssignmentStatement("val_0", z3.BitVecVal(1, 8)),
                    P4Exit(),]
                )                )), 
P4Declaration("do_action_ingress_0", P4Action("do_action_ingress", params=[],                 body=BlockStatement([
                    MethodCallStmt(MethodCallExpr("do_action_0", [], P4Member(P4Member("h", "h"), "a"), )),]
                )                )), ]
        ))
    )
    prog_state.declare_global(
