...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        HeaderType("H", prog_state, fields=[("ttl", z3.BitVecSort(8)), ], type_params=[])
    )
    prog_state.declare_global(
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                AssignmentStatement(P4Member(P4Member("h", "h"), "ttl"), P4sub(P4Member(P4Member("h", "h"), "ttl"), 1)),
                IfStatement(P4gt(P4Member(P4Member("h", "h"), "ttl"), 0), BlockStatement([
                    AssignmentStatement(P4Member("sm", "egress_spec"), 1),]
                ), BlockStatement([
                    MethodCallStmt(MethodCallExpr("mark_to_drop", [], "sm", )),]
                )),]
            ),
            local_decls=[]
        ))
