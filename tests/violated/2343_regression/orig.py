...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        P4Declaration("do_function", P4Function("do_function", return_type=z3.BitVecSort(16), params=[
            P4Parameter("inout", "val", z3.BitVecSort(48), None),],         body=BlockStatement([
            AssignmentStatement("val", z3.BitVecVal(1, 48)),
            P4Return(z3.BitVecVal(1, 16)),]
        )        )        )
    )
    prog_state.declare_global(
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                AssignmentStatement(P4Member(P4Member("h", "eth_hdr"), "eth_type"), P4Slice(P4lshift(MethodCallExpr("do_function", [], P4Member(P4Member("h", "eth_hdr"), "dst_addr"), ), z3.BitVecVal(15, 8)), 15, 0)),]
            ),
            local_decls=[]
        ))
    )
