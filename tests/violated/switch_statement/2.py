...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr(P4Member("packet", "emit"), [], "hdr", )),]
            ),
            local_decls=[]
        ))
    )
    prog_state.declare_global(
        ControlDeclaration(P4Parser(
            ...
            body=ParserTree([
                ...
                components=[
                MethodCallStmt(MethodCallExpr(P4Member("pkt", "extract"), [], P4Member("hdr", "eth"), )),                ]),
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                SwitchStatement(P4Member(MethodCallExpr(P4Member("simple_table", "apply"), [], ), "action_run"),cases=[("do_something", BlockStatement([
                    AssignmentStatement("tmp_condition", z3.BitVecVal(0, 8)),]
                )), ]),
                IfStatement(P4gt("tmp_condition", 0), BlockStatement([
                    AssignmentStatement(P4Member(P4Member("hdr", "h"), "a"), z3.BitVecVal(0, 8)),]
                ), P4Noop()),]
            ),
            local_decls=[
ValueDeclaration("tmp_condition", P4Initializer(z3.BitVecVal(0, 8), z3.BitVecSort(8)), z3_type=z3.BitVecSort(8)), 
P4Declaration("do_something", P4Action("do_something", params=[],                 body=BlockStatement([
                    ...
                    AssignmentStatement("tmp_condition", z3.BitVecVal(1, 8)),]
                )                )), 
P4Declaration("simple_table", P4Table("simple_table", key=[(P4Member(P4Member("hdr", "h"), "b"), "exact"), ], actions=[MethodCallExpr("NoAction", [], ), MethodCallExpr("do_something", [], ), ], default_action="NoAction", immutable=False)), ]
        ))
    )
    prog_state.declare_global(
        ControlDeclaration(P4Control(
