...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                IfStatement(P4eq(z3.BitVecVal(255, 8), P4neg(1)), BlockStatement([
                    AssignmentStatement(P4Member(P4Member("h", "eth_hdr"), "eth_type"), 1),]
                ), BlockStatement([
                    AssignmentStatement(P4Member(P4Member("h", "eth_hdr"), "eth_type"), 2),]
                )),]
            ),
            local_decls=[]
        ))
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr(P4Member("pkt", "emit"), [], "h", )),]
            ),
            local_decls=[]
        ))
    )
