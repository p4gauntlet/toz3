...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        P4Declaration("do_something", P4Function("do_something", return_type=None, params=[
            P4Parameter("inout", "val", z3.BitVecSort(8), None),],         body=BlockStatement([
            IfStatement(P4eq("val", z3.BitVecVal(0, 8)), BlockStatement([
                AssignmentStatement("val", z3.BitVecVal(1, 8)),
                P4Return(None),]
            ), P4Noop()),
            AssignmentStatement("val", z3.BitVecVal(2, 8)),
            P4Return(None),]
        )        )        )
    )
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            type_params=[],
            params=[
                P4Parameter("inout", "h", "Parsed_packet", None),
                P4Parameter("inout", "m", "Metadata", None),
                P4Parameter("inout", "sm", "standard_metadata_t", None),],
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr("do_something", [], P4Member(P4Member("h", "h"), "b"), )),]
            ),
            local_decls=[]
        ))
    )
