...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        HeaderType("hdr", prog_state, fields=[("e", z3.BitVecSort(8)), ("t", z3.BitVecSort(16)), ("l", z3.BitVecSort(8)), ("r", z3.BitVecSort(8)), ("v", z3.BitVecSort(8)), ], type_params=[])
    )
    prog_state.declare_global(
        StructType("Header_t", prog_state, fields=[("h", "hdr"), ], type_params=[])
    )
    prog_state.declare_global(
        StructType("Meta_t", prog_state, fields=[], type_params=[])
    )
    prog_state.declare_global(
        ControlDeclaration(P4Parser(
            ...
            params=[
                P4Parameter("none", "b", "packet_in", None),
                P4Parameter("out", "h", "Header_t", None),
                P4Parameter("inout", "m", "Meta_t", None),
            ...
            body=ParserTree([
                ...
                components=[
                MethodCallStmt(MethodCallExpr(P4Member("b", "extract"), ["hdr", ], P4Member("h", "h"), )),                ]),
                ])
))
    )
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            type_params=[],
            params=[
                P4Parameter("inout", "h", "Header_t", None),
                P4Parameter("inout", "m", "Meta_t", None),],
            const_params=[],
            body=BlockStatement([]
            ),
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            params=[
                P4Parameter("none", "b", "packet_out", None),
                P4Parameter("in", "h", "Header_t", None),],
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr(P4Member("b", "emit"), ["hdr", ], P4Member("h", "h"), )),]
            ),
            local_decls=[]
        ))
    )
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr(P4Member("t_exact_0", "apply"), [], )),]
            ),
            local_decls=[
P4Declaration("a", P4Action("a", params=[],                 body=BlockStatement([
                    AssignmentStatement(P4Member("standard_meta", "egress_spec"), z3.BitVecVal(0, 9)),]
                )                )), 
P4Declaration("a_with_control_params", P4Action("a_with_control_params", params=[
                    P4Parameter("none", "x", z3.BitVecSort(9), None),],                 body=BlockStatement([
                    AssignmentStatement(P4Member("standard_meta", "egress_spec"), "x"),]
                )                )), 
P4Declaration("t_exact_0", P4Table("t_exact", key=[(P4Member(P4Member("h", "h"), "e"), "exact"), ], actions=[MethodCallExpr("a", [], ), MethodCallExpr("a_with_control_params", [], ), ], default_action=MethodCallExpr("a", [], ), entries=[([z3.BitVecVal(1, 8), ], MethodCallExpr("a_with_control_params", [], z3.BitVecVal(1, 9), )), ([z3.BitVecVal(2, 8), ], MethodCallExpr("a_with_control_params", [], z3.BitVecVal(2, 9), )), ], immutable=True)), ]
        ))
    )
    prog_state.declare_global(
        InstanceDeclaration("main", TypeSpecializer("V1Switch", "Header_t", "Meta_t", ), ConstCallExpr("p", ), ConstCallExpr("vrfy", ), ConstCallExpr("ingress", ), ConstCallExpr("egress", ), ConstCallExpr("update", ), ConstCallExpr("deparser", ), )
    )
