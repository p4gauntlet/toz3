...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            body=BlockStatement([
                BlockStatement([
                    AssignmentStatement("tmp", P4Member(MethodCallExpr(P4Member("simple_table_0", "apply"), [], ), "hit")),
                    IfStatement("tmp", BlockStatement([
                        AssignmentStatement("tmp_0", z3.BitVecVal(1, 16)),]
                    ), BlockStatement([
                        AssignmentStatement("tmp_0", z3.BitVecVal(2, 16)),]
                    )),
                    AssignmentStatement("tmp_1", "tmp_0"),
                    MethodCallStmt(MethodCallExpr("dummy_action_0", [], P4Member(P4Member("h", "eth_hdr"), "eth_type"), "tmp_1", )),]
                ),]
            ),
            local_decls=[
P4Declaration("simple_action_0", P4Action("simple_action", params=[],                 body=BlockStatement([
                    AssignmentStatement(P4Member(P4Member("h", "eth_hdr"), "eth_type"), z3.BitVecVal(3, 16)),]
                )                )), 
P4Declaration("dummy_action_0", P4Action("dummy_action", params=[
                    P4Parameter("inout", "val1", z3.BitVecSort(16), None),
                    P4Parameter("in", "val2", z3.BitVecSort(16), None),],                 body=BlockStatement([]
                )                )), 
P4Declaration("simple_table_0", P4Table("simple_table", key=[(P4Member(P4Member("h", "eth_hdr"), "src_addr"), "exact"), ], actions=[MethodCallExpr("simple_action_0", [], ), ], default_action=MethodCallExpr("simple_action_0", [], ), immutable=False)), 
ValueDeclaration("tmp", None, z3_type=z3.BoolSort()), 
ValueDeclaration("tmp_0", None, z3_type=z3.BitVecSort(16)), 
