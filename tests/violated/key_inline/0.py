...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            local_decls=[
P4Declaration("c_a", P4Action("a", params=[],                 body=BlockStatement([
                    AssignmentStatement(P4Member(P4Member("h", "h"), "b"), P4add(P4Member(P4Member("h", "h"), "a"), 1)),]
                )                )), 
P4Declaration("t_0", P4Table("t", key=[(P4add(P4Member(P4Member("h", "h"), "a"), P4Member(P4Member("h", "h"), "a")), "exact"), ], actions=[MethodCallExpr("c_a", [], ), MethodCallExpr("NoAction", [], ), ], default_action=MethodCallExpr("NoAction", [], ), immutable=False)), ]
        ))
    )
    prog_state.declare_global(
