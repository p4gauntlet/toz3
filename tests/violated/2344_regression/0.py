...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        HeaderType("H", prog_state, fields=[("a", z3.BitVecSort(32)), ], type_params=[])
    )
    prog_state.declare_global(
...
P4Declaration("simple_action", P4Action("simple_action", params=[],                 body=BlockStatement([
                    BlockStatement([
                        ...
                        IfStatement(P4not(MethodCallExpr(P4Member("tmp1_0", "isValid"), [], )), BlockStatement([
                            MethodCallStmt(MethodCallExpr(P4Member("tmp1_0", "setValid"), [], )),
                            AssignmentStatement(P4Member("tmp1_0", "a"), z3.BitVecVal(10, 32)),]
                        ), BlockStatement([
                            AssignmentStatement(P4Member("tmp1_0", "a"), P4add(P4Member("tmp1_0", "a"), z3.BitVecVal(15, 32))),]
                        )),
                        AssignmentStatement("hasReturned", z3.BoolVal(True)),
                        AssignmentStatement("retval", P4Member("tmp1_0", "a")),
                        AssignmentStatement(P4Member(P4Member("h", "h"), "a"), "retval"),]
                    ),]
                )                )), 
P4Declaration("simple_table_0", P4Table("simple_table", key=[], actions=[MethodCallExpr("simple_action", [], ), ], default_action=MethodCallExpr("simple_action", [], ), immutable=False)), ]
        ))
    )
    prog_state.declare_global(
