...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr("do_thing", [], "c", )),
                MethodCallStmt(MethodCallExpr("do_thing", [], "c", )),]
            ),
            local_decls=[
ValueDeclaration("c", P4Initializer(z3.BitVecVal(12, 8), z3.BitVecSort(8)), z3_type=z3.BitVecSort(8)), 
P4Declaration("do_thing", P4Action("do_thing", params=[
                    P4Parameter("inout", "inout_c", z3.BitVecSort(8), None),],                 body=BlockStatement([
                    AssignmentStatement(P4Member(P4Member("h", "h"), "a"), "inout_c"),
                    AssignmentStatement("inout_c", z3.BitVecVal(24, 8)),]
                )                )), ]
        ))
    )
    prog_state.declare_global(
