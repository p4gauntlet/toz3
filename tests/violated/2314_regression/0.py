...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        HeaderType("I", prog_state, fields=[("etherType", z3.BitVecSort(16)), ], type_params=[])
    )
    prog_state.declare_global(
        StructType("h", prog_state, fields=[("ether", "ethernet_t"), ("h", "H"), ("i", "I"), ], type_params=[])
    )
    prog_state.declare_global(
        StructType("m", prog_state, fields=[], type_params=[])
    )
    prog_state.declare_global(
        ControlDeclaration(P4Parser(
            ...
            const_params=[],
            local_decls=[
ValueDeclaration("etherType_0", None, z3_type=z3.BitVecSort(16)), ],
            body=ParserTree([
                ParserState(name="start", select=ParserSelect(["etherType_0", ], [(z3.BitVecVal(2048, 16), "h"), (z3.BitVecVal(33024, 16), "i"), (DefaultExpression(), "accept"), ]),
                components=[
                AssignmentStatement("etherType_0", P4Member(P4Member("hdr", "ether"), "etherType")),                ]),
                ParserState(name="h", select="accept",
                components=[
                MethodCallStmt(MethodCallExpr(P4Member("b", "extract"), ["H", ], P4Member("hdr", "h"), )),                ]),
                ParserState(name="i", select="start",
                components=[
                MethodCallStmt(MethodCallExpr(P4Member("b", "extract"), ["I", ], P4Member("hdr", "i"), )),
                AssignmentStatement("etherType_0", P4Member(P4Member("hdr", "i"), "etherType")),                ]),
                ])
))
    )
    prog_state.declare_global(
        ControlDeclaration(P4Parser(
            ...
            params=[
                ...
                P4Parameter("inout", "meta", "m", None),
                P4Parameter("inout", "std", "standard_metadata_t", None),],
            const_params=[],
            local_decls=[
InstanceDeclaration("l3_0", "L3", ), ],
            body=ParserTree([
                ParserState(name="start", select="accept",
                components=[
                MethodCallStmt(MethodCallExpr(P4Member("b", "extract"), ["ethernet_t", ], P4Member("hdr", "ether"), )),
                MethodCallStmt(MethodCallExpr(P4Member("l3_0", "apply"), [], "b", "hdr", )),                ]),
                ])
))
    )
    prog_state.declare_global(
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            params=[
                P4Parameter("none", "b", "packet_out", None),
                P4Parameter("in", "hdr", "h", None),],
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr(P4Member("b", "emit"), ["h", ], "hdr", )),]
            ),
            local_decls=[]
        ))
    )
    prog_state.declare_global(
        InstanceDeclaration("main", TypeSpecializer("V1Switch", "h", "m", ), ConstCallExpr("MyParser", ), ConstCallExpr("MyVerifyChecksum", ), ConstCallExpr("MyIngress", ), ConstCallExpr("MyEgress", ), ConstCallExpr("MyComputeChecksum", ), ConstCallExpr("MyDeparser", ), )
    )
