...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        HeaderType("Ethernet_h", prog_state, fields=[("dstAddr", "EthernetAddress"), ("srcAddr", "EthernetAddress"), ("etherType", z3.BitVecSort(16)), ], type_params=[])
    )
    prog_state.declare_global(
        HeaderType("H", prog_state, fields=[("a", z3.BitVecSort(4)), ("b", z3.BitVecSort(4)), ], type_params=[])
    )
    prog_state.declare_global(
        StructType("Headers", prog_state, fields=[("ethernet", "Ethernet_h"), ("h", "H"), ], type_params=[])
    )
    prog_state.declare_global(
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Parser(
            ...
            body=ParserTree([
                ...
                components=[
                MethodCallStmt(MethodCallExpr(P4Member("pkt", "extract"), ["Ethernet_h", ], P4Member("hdr", "ethernet"), )),
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr("foo", [], )),]
            ),
            local_decls=[
ValueDeclaration("hasReturned", None, z3_type=z3.BoolSort()), 
P4Declaration("foo", P4Action("foo", params=[],                 body=BlockStatement([
                    AssignmentStatement("hasReturned", z3.BoolVal(False)),
                    AssignmentStatement(P4Member(P4Member("hdr", "h"), "b"), P4add(P4Member(P4Member("hdr", "h"), "b"), z3.BitVecVal(5, 4))),
                    BlockStatement([
                        AssignmentStatement(P4Member(P4Member("hdr", "h"), "b"), P4Mux(P4gt(P4Member(P4Member("hdr", "h"), "b"), z3.BitVecVal(10, 4)), P4xor(P4Member(P4Member("hdr", "h"), "b"), z3.BitVecVal(5, 4)), P4Member(P4Member("hdr", "h"), "b"))),
                        AssignmentStatement("hasReturned", P4Mux(P4gt(P4Member(P4Member("hdr", "h"), "b"), z3.BitVecVal(10, 4)), z3.BoolVal(True), "hasReturned")),]
                    ),
                    BlockStatement([
                        AssignmentStatement(P4Member(P4Member("hdr", "h"), "b"), P4Mux(P4not("hasReturned"), P4add(P4Member(P4Member("hdr", "h"), "b"), z3.BitVecVal(5, 4)), P4Member(P4Member("hdr", "h"), "b"))),]
                    ),]
                )                )), ]
        ))
    )
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            body=BlockStatement([
                BlockStatement([
                    MethodCallStmt(MethodCallExpr(P4Member("packet", "emit"), ["Ethernet_h", ], P4Member("hdr", "ethernet"), )),
    ...
    prog_state.declare_global(
        InstanceDeclaration("main", TypeSpecializer("V1Switch", "Headers", "Metadata", ), ConstCallExpr("p", ), ConstCallExpr("vrfy", ), ConstCallExpr("ingress", ), ConstCallExpr("egress", ), ConstCallExpr("update", ), ConstCallExpr("deparser", ), )
    )
