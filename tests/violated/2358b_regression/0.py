...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            body=BlockStatement([
                BlockStatement([
                    AssignmentStatement("tmp", "h"),
                    MethodCallStmt(MethodCallExpr("reset_action_0", [], "tmp", "tmp_0", )),
                    AssignmentStatement("h", "tmp_0"),]
                ),]
            ),
            local_decls=[
P4Declaration("reset_action_0", P4Action("reset_action", params=[
                    P4Parameter("in", "in_hdr", "Headers", None),
                    P4Parameter("out", "out_hdr", "Headers", None),],                 body=BlockStatement([
                    P4Exit(),]
                )                )), 
