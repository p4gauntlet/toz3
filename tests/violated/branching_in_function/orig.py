...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        P4Declaration("do_thing", P4Function("do_thing", return_type=z3.BitVecSort(48), params=[
            P4Parameter("inout", "val_0", z3.BitVecSort(16), None),],         body=BlockStatement([
            IfStatement(P4ge("val_0", z3.BitVecVal(2, 16)), BlockStatement([
                AssignmentStatement("val_0", z3.BitVecVal(0, 16)),]
            ), BlockStatement([
                AssignmentStatement("val_0", z3.BitVecVal(1, 16)),]
            )),
            P4Return(2),]
        )        )        )
    )
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                AssignmentStatement(P4Member(P4Member("h", "eth_hdr"), "dst_addr"), MethodCallExpr("do_thing", [], P4Member(P4Member("h", "eth_hdr"), "eth_type"), )),]
            ),
            local_decls=[]
        ))
    )
