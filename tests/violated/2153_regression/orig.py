...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            body=BlockStatement([
                BlockStatement([
                    MethodCallStmt(MethodCallExpr(P4Member("packet", "emit"), ["ethernet_t", ], P4Member("hdr", "eth"), )),
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                AssignmentStatement("tmp_condition_0", z3.BitVecVal(0, 8)),
                SwitchStatement(P4Member(MethodCallExpr(P4Member("simple_table_0", "apply"), [], ), "action_run"),cases=[("do_something", BlockStatement([
                    AssignmentStatement("tmp_condition_0", z3.BitVecVal(1, 8)),]
                )), ]),
                IfStatement(P4gt("tmp_condition_0", z3.BitVecVal(0, 8)), BlockStatement([
                    AssignmentStatement(P4Member(P4Member("hdr", "h"), "a"), z3.BitVecVal(0, 8)),]
                ), P4Noop()),]
            ),
            local_decls=[
ValueDeclaration("tmp_condition_0", None, z3_type=z3.BitVecSort(8)), 
...
P4Declaration("simple_table_0", P4Table("simple_table", key=[(P4Member(P4Member("hdr", "h"), "b"), "exact"), ], actions=[MethodCallExpr("NoAction_0", [], ), MethodCallExpr("do_something", [], ), ], default_action=MethodCallExpr("NoAction_0", [], ), immutable=False)), ]
        ))
    )
    prog_state.declare_global(
        ControlDeclaration(P4Control(
