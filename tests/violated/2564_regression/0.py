...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        StructType("Headers", prog_state, fields=[("eth_hdr", "ethernet_t"), ("h", prog_state.stack("H", 2)), ], type_params=[])
    )
    prog_state.declare_global(
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Parser(
            ...
            body=ParserTree([
                ...
                MethodCallStmt(MethodCallExpr(P4Member("pkt", "extract"), ["H", ], P4Member(P4Member("hdr", "h"), "next"), )),
                MethodCallStmt(MethodCallExpr(P4Member("pkt", "extract"), ["H", ], P4Member(P4Member("hdr", "h"), "next"), )),                ]),
                ])
))
    )
...
P4Declaration("simple_action", P4Action("simple_action", params=[],                 body=BlockStatement([
                    BlockStatement([
                        ...
                        AssignmentStatement("cond", P4ne(P4Member(P4Member("h", "eth_hdr"), "src_addr"), P4Member(P4Member("h", "eth_hdr"), "dst_addr"))),
                        AssignmentStatement(P4Member(P4Index(P4Member("h", "h"), 1), "a"), P4Mux("cond", P4Member(P4Index(P4Member("h", "h"), 1), "a"), z3.BitVecVal(1, 8))),]
                    ),]
                )                )), ]
        ))
    )
        ...
        ControlDeclaration(P4Control(
            ...
            body=BlockStatement([
                BlockStatement([
                    ...
                    MethodCallStmt(MethodCallExpr(P4Member("b", "emit"), ["H", ], P4Index(P4Member("h", "h"), 0), )),
                    MethodCallStmt(MethodCallExpr(P4Member("b", "emit"), ["H", ], P4Index(P4Member("h", "h"), 1), )),]
                ),]
            ),
            local_decls=[]
        ))
