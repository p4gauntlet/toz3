...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        HeaderType("H", prog_state, fields=[("a", z3.BitVecSort(32)), ("b", z3.BitVecSort(32)), ], type_params=[])
    )
    prog_state.declare_global(
    ...
    prog_state.declare_global(
        P4Declaration("test_func", P4Function("test_func", return_type=z3.BitVecSort(32), params=[],         body=BlockStatement([
            P4Return(z3.BitVecVal(2, 32)),]
        )        )        )
    )
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                AssignmentStatement(P4Member(P4Member("hdr", "h"), "a"), z3.BitVecVal(1, 32)),
                MethodCallStmt(MethodCallExpr("update_value_0", [], "value_0", )),]
            ),
            local_decls=[
ValueDeclaration("value_0", None, z3_type=z3.BitVecSort(32)), 
P4Declaration("update_value_0", P4Action("update_value", params=[
                    P4Parameter("out", "value", z3.BitVecSort(32), None),],                 body=BlockStatement([
                    AssignmentStatement(P4Member(P4Member("hdr", "h"), "a"), MethodCallExpr("test_func", [], )),]
                )                )), ]
        ))
    )
    prog_state.declare_global(
    ...
    prog_state.declare_global(
        InstanceDeclaration("main", TypeSpecializer("V1Switch", "headers", "metadata", ), ConstCallExpr("ParserImpl", ), ConstCallExpr("VerifyChecksumImpl", ), ConstCallExpr("IngressImpl", ), ConstCallExpr("EgressImpl", ), ConstCallExpr("ComputeChecksumImpl", ), ConstCallExpr("DeparserImpl", ), )
    )
