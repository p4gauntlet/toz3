...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            local_decls=[
ValueDeclaration("tmp_val_0", None, z3_type=z3.BitVecSort(48)), 
...
P4Declaration("simple_action", P4Action("simple_action", params=[],                 body=BlockStatement([
                    BlockStatement([
                        BlockStatement([
                            BlockStatement([
                                AssignmentStatement("tmp", P4Mux(P4ne(P4Member(P4Member("h", "eth_hdr"), "eth_type"), z3.BitVecVal(0, 16)), P4Mux(P4ne(P4Member(P4Member("h", "eth_hdr"), "src_addr"), z3.BitVecVal(0, 48)), z3.BitVecVal(1, 48), z3.BitVecVal(2, 48)), "tmp")),]
                            ),]
                        ),
                        AssignmentStatement(P4Member(P4Member("h", "eth_hdr"), "dst_addr"), P4Mux(P4ne(P4Member(P4Member("h", "eth_hdr"), "eth_type"), z3.BitVecVal(0, 16)), "tmp_val_0", P4Member(P4Member("h", "eth_hdr"), "dst_addr"))),]
                    ),]
                )                )), ]
        ))
    )
