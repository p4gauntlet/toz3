...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            body=BlockStatement([
                BlockStatement([
                    AssignmentStatement("tmp", P4Member(P4Member("h", "eth_hdr"), "src_addr")),
                    AssignmentStatement("tmp_0", P4Member(P4Member("h", "eth_hdr"), "src_addr")),
                    MethodCallStmt(MethodCallExpr("call_exit_0", [], "tmp", "tmp_0", )),
                    AssignmentStatement(P4Member(P4Member("h", "eth_hdr"), "src_addr"), "tmp"),]
                ),]
            ),
            local_decls=[
P4Declaration("call_exit_0", P4Action("call_exit", params=[
                    P4Parameter("inout", "val", z3.BitVecSort(48), None),
                    P4Parameter("in", "val1", z3.BitVecSort(48), None),],                 body=BlockStatement([
                    AssignmentStatement("val", z3.BitVecVal(1, 48)),
                    P4Exit(),]
                )                )), 
ValueDeclaration("tmp", None, z3_type=z3.BitVecSort(48)), 
ValueDeclaration("tmp_0", None, z3_type=z3.BitVecSort(48)), ]
        ))
    )
    prog_state.declare_global(
        ControlDeclaration(P4Control(
