...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        StructType("headers", prog_state, fields=[("h", "H"), ("b", "H"), ], type_params=[])
    )
    prog_state.declare_global(
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                IfStatement(P4eq(P4Member(P4Member("hdr", "h"), "s"), P4Member(P4Member("hdr", "b"), "s")), BlockStatement([
                    AssignmentStatement(P4Member(P4Member("hdr", "h"), "s"), P4add(P4Member(P4Member("hdr", "h"), "s"), z3.BitVecVal(1, 8))),]
                ), P4Noop()),]
            ),
            local_decls=[]
        ))
