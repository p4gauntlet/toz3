...
P4Declaration("simple_action", P4Action("simple_action", params=[],                 body=BlockStatement([
                    ...
                    IfStatement(P4eq(P4Member(P4Member("h", "eth_hdr"), "eth_type"), z3.BitVecVal(1, 16)), BlockStatement([
                        AssignmentStatement("hasReturned", z3.BoolVal(True)),]
                    ), P4Noop()),
                    IfStatement(P4not("hasReturned"), BlockStatement([
                        AssignmentStatement(P4Member(P4Member("h", "eth_hdr"), "src_addr"), z3.BitVecVal(1, 48)),
                        BlockStatement([
                            BlockStatement([
                                AssignmentStatement("val_eth_hdr", P4Member("h", "eth_hdr")),]
                            ),
                            BlockStatement([
                                AssignmentStatement(P4Member("h", "eth_hdr"), "val_eth_hdr"),]
                            ),]
                        ),]
                    ), P4Noop()),]
                )                )), ]
