...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        HeaderType("H", prog_state, fields=[("a", z3.BitVecSort(16)), ], type_params=[])
    )
    prog_state.declare_global(
    ...
    prog_state.declare_global(
        P4Declaration("set", P4Function("set", return_type=z3.BitVecSort(48), params=[
            P4Parameter("inout", "set", z3.BitVecSort(48), None),],         body=BlockStatement([
            AssignmentStatement("set", z3.BitVecVal(1, 48)),
            P4Return(z3.BitVecVal(2, 48)),]
        )        )        )
    )
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            body=BlockStatement([
                BlockStatement([
                    AssignmentStatement("tmp", MethodCallExpr("set", [], P4Member(P4Member("h", "eth_hdr"), "dst_addr"), )),
                    AssignmentStatement("tmp_0", P4Cast(P4Initializer([P4Cast(P4Initializer([P4Member(P4Member("h", "eth_hdr"), "dst_addr"), "tmp", z3.BitVecVal(1, 16), ], ), "ethernet_t"), ], ), "Headers")),]
                ),
                AssignmentStatement("h", "tmp_0"),]
            ),
            local_decls=[
