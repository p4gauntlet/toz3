...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        HeaderType("H", prog_state, fields=[("a", z3.BitVecSort(8)), ("b", z3.BitVecSort(8)), ], type_params=[])
    )
    prog_state.declare_global(
        StructType("Headers", prog_state, fields=[("eth", "ethernet_t"), ("h", "H"), ], type_params=[])
    )
    prog_state.declare_global(
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr(P4Member("packet", "emit"), ["Headers", ], "hdr", )),]
            ),
            local_decls=[]
        ))
    )
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            params=[
                ...
                P4Parameter("inout", "m", "Metadata", None),
                P4Parameter("inout", "sm", "standard_metadata_t", None),],
            const_params=[],
            body=BlockStatement([
                BlockStatement([
                    AssignmentStatement("tmp", P4Member(P4Member("h", "h"), "b")),
                    AssignmentStatement("tmp_0", P4Member(P4Member("h", "h"), "b")),
                    MethodCallStmt(MethodCallExpr("do_action", [], "tmp", "tmp_0", P4Member(P4Member("h", "h"), "b"), )),
                    AssignmentStatement(P4Member(P4Member("h", "h"), "b"), "tmp"),
                    AssignmentStatement(P4Member(P4Member("h", "h"), "b"), "tmp_0"),]
                ),
                IfStatement(P4gt(P4Member(P4Member("h", "h"), "b"), z3.BitVecVal(1, 8)), BlockStatement([
                    AssignmentStatement(P4Member(P4Member("h", "h"), "a"), z3.BitVecVal(1, 8)),]
                ), P4Noop()),]
            ),
            local_decls=[
P4Declaration("do_action", P4Action("do_action_2", params=[
                    P4Parameter("inout", "val_0", z3.BitVecSort(8), None),
                    P4Parameter("inout", "val_1", z3.BitVecSort(8), None),
                    P4Parameter("inout", "val_2", z3.BitVecSort(8), None),],                 body=BlockStatement([
                    AssignmentStatement("val_1", z3.BitVecVal(2, 8)),]
                )                )), 
ValueDeclaration("tmp", None, z3_type=z3.BitVecSort(8)), 
ValueDeclaration("tmp_0", None, z3_type=z3.BitVecSort(8)), ]
        ))
    )
    prog_state.declare_global(
        ControlDeclaration(P4Control(
