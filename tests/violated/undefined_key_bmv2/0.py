...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr(P4Member("c_t", "apply"), [], )),
...
P4Declaration("c_a_0", P4Action("a", params=[],                 body=BlockStatement([
                    AssignmentStatement(P4Member(P4Member("h", "h"), "b"), P4Member(P4Member("h", "h"), "a")),]
                )                )), 
P4Declaration("c_t", P4Table("t", key=[(P4add(P4Member(P4Member("h", "h"), "b"), P4Member(P4Member("h", "h"), "a")), "exact"), ], actions=[MethodCallExpr("c_a_0", [], ), MethodCallExpr("NoAction_0", [], ), ], default_action=MethodCallExpr("NoAction_0", [], ), immutable=False)), ]
        ))
    )
    prog_state.declare_global(
