...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        HeaderType("H", prog_state, fields=[("a", z3.BitVecSort(4)), ], type_params=[])
    )
    prog_state.declare_global(
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Parser(
            ...
            local_decls=[],
            body=ParserTree([
                ParserState(name="start", select=ParserSelect([P4Member(P4Member("hdr", "h"), "a"), ], [(P4Mask(z3.BitVecVal(4, 4), z3.BitVecVal(0, 4)), "reject"), (P4Mask(z3.BitVecVal(1, 4), z3.BitVecVal(0, 4)), "accept"), (P4Mask(z3.BitVecVal(2, 4), z3.BitVecVal(0, 4)), "accept"), (DefaultExpression(), "reject"), ]),
                components=[
                MethodCallStmt(MethodCallExpr(P4Member("packet", "extract"), ["H", ], hdr=P4Member("hdr", "h"), )),                ]),
                ])
))
    )
    prog_state.declare_global(
