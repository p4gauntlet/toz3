...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr("slice_action_0", [], P4Slice(P4Member(P4Member("h", "h"), "a"), 0, 0), )),]
            ),
            local_decls=[
P4Declaration("slice_action_0", P4Action("slice_action", params=[
                    P4Parameter("inout", "sliced_val", z3.BitVecSort(1), None),],                 body=BlockStatement([
                    AssignmentStatement(P4Member(P4Member("h", "h"), "a"), z3.BitVecVal(2, 8)),
                    AssignmentStatement("sliced_val", z3.BitVecVal(1, 1)),]
                )                )), ]
        ))
    )
    prog_state.declare_global(
