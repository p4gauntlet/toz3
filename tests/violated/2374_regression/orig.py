...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr("simple_action_0", [], P4Member(P4Member("h", "eth_hdr"), "eth_type"), )),]
            ),
            local_decls=[
P4Declaration("exit_action_0", P4Action("exit_action", params=[],                 body=BlockStatement([
                    P4Exit(),]
                )                )), 
P4Declaration("assignment_action_0", P4Action("assignment_action", params=[
                    P4Parameter("out", "val", z3.BitVecSort(16), None),],                 body=BlockStatement([
                    AssignmentStatement("val", z3.BitVecVal(48879, 16)),]
                )                )), 
P4Declaration("simple_action_0", P4Action("simple_action", params=[
                    P4Parameter("inout", "val", z3.BitVecSort(16), None),],                 body=BlockStatement([
                    AssignmentStatement("val", z3.BitVecVal(57005, 16)),
                    MethodCallStmt(MethodCallExpr("exit_action_0", [], )),
                    MethodCallStmt(MethodCallExpr("assignment_action_0", [], "val", )),]
                )                )), ]
        ))
    )
    prog_state.declare_global(
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr(P4Member("pkt", "emit"), ["Headers", ], "h", )),]
            ),
            local_decls=[]
        ))
    )
