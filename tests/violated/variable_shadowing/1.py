...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        HeaderType("hdr", prog_state, fields=[("a", z3.BitVecSort(32)), ("b", z3.BitVecSort(32)), ], type_params=[])
    )
    prog_state.declare_global(
        StructType("Headers", prog_state, fields=[("h", "hdr"), ], type_params=[])
    )
    prog_state.declare_global(
        StructType("Meta", prog_state, fields=[("test", z3.BitVecSort(8)), ], type_params=[])
    )
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            type_params=[],
            params=[
                P4Parameter("inout", "h", "hdr", None),],
            const_params=[],
            body=BlockStatement([
                ValueDeclaration("tmp", P4Initializer(0, z3.BitVecSort(9)), z3_type=z3.BitVecSort(9)),
                ValueDeclaration("m", P4Initializer([0, ], "Meta"), z3_type="Meta"),
                MethodCallStmt(MethodCallExpr(P4Member("t_0", "apply"), [], )),
                AssignmentStatement("tmp", 0),]
            ),
            local_decls=[
P4Declaration("a_0", P4Action("a", params=[],                 body=BlockStatement([
                    AssignmentStatement(P4Member("h", "b"), P4Member("h", "a")),]
                )                )), 
P4Declaration("t_0", P4Table("t", key=[(P4add(P4Member("h", "a"), P4Member("h", "a")), "exact"), ], actions=[MethodCallExpr("a_0", [], ), MethodCallExpr("NoAction", [], ), ], default_action=MethodCallExpr("NoAction", [], ), immutable=False)), ]
        ))
    )
    prog_state.declare_global(
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                AssignmentStatement(P4Member("m", "test"), 1),
                ValueDeclaration("tmp", P4Initializer(1, z3.BitVecSort(9)), z3_type=z3.BitVecSort(9)),
                MethodCallStmt(MethodCallExpr(P4Member("c_0", "apply"), [], P4Member("h", "h"), )),
                AssignmentStatement(P4Member("sm", "egress_spec"), "tmp"),]
            ),
            local_decls=[
InstanceDeclaration("c_0", "compute", ), ]
        ))
    )
    prog_state.declare_global(
