...
def p4_program(prog_state):
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr("do_action", [], )),]
            ),
            local_decls=[
ValueDeclaration("pointless_bool_0", None, z3_type=z3.BoolSort()), 
ValueDeclaration("tmp", None, z3_type=z3.BitVecSort(48)), 
ValueDeclaration("hasReturned", None, z3_type=z3.BoolSort()), 
ValueDeclaration("val_0", None, z3_type=z3.BitVecSort(16)), 
P4Declaration("do_action", P4Action("do_action", params=[],                 body=BlockStatement([
                    AssignmentStatement("hasReturned", z3.BoolVal(False)),
                    AssignmentStatement("pointless_bool_0", z3.BoolVal(True)),
                    BlockStatement([
                        ValueDeclaration("cond", None, z3_type=z3.BoolSort()),
                        AssignmentStatement("cond", P4ne(P4Member(P4Member("h", "eth_hdr"), "dst_addr"), z3.BitVecVal(0, 48))),
                        AssignmentStatement("hasReturned", P4Mux("cond", "hasReturned", z3.BoolVal(True))),]
                    ),
                    BlockStatement([
                        ValueDeclaration("cond_0", None, z3_type=z3.BoolSort()),
                        AssignmentStatement("cond_0", P4not("hasReturned")),
                        BlockStatement([
                            BlockStatement([
                                AssignmentStatement(P4Member(P4Member("h", "eth_hdr"), "eth_type"), P4Mux("cond_0", "val_0", P4Member(P4Member("h", "eth_hdr"), "eth_type"))),
                                AssignmentStatement("tmp", P4Mux("cond_0", P4Mux("pointless_bool_0", z3.BitVecVal(1, 48), z3.BitVecVal(2, 48)), "tmp")),]
                            ),]
                        ),
                        BlockStatement([
                            AssignmentStatement("val_0", P4Mux("cond_0", P4Member(P4Member("h", "eth_hdr"), "eth_type"), "val_0")),]
                        ),
                        AssignmentStatement(P4Member(P4Member("h", "eth_hdr"), "src_addr"), P4Mux("cond_0", "tmp", P4Member(P4Member("h", "eth_hdr"), "src_addr"))),]
                    ),]
                )                )), ]
        ))
    )
    ...
    prog_state.declare_global(
        ControlDeclaration(P4Control(
            ...
            body=BlockStatement([
                BlockStatement([
                    MethodCallStmt(MethodCallExpr(P4Member("pkt", "emit"), ["ethernet_t", ], P4Member("h", "eth_hdr"), )),]
                ),]
            ),
            local_decls=[]
        ))
